...
def main_worker(gpu, ngpus_per_node, args):
    ...
    '''
    Kangjun use this for test images
    '''
    ...
    train_size = int(0.8 * len(train_dataset))  # 90% of the dataset for training
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, avg = validate(val_loader, model, criterion,
                        args, tf_writer, epoch=epoch)

        # remember best acc@1 and save checkpoint
        is_best = avg > best_avg
        best_acc1 = max(acc1, best_acc1)
        best_avg = max(avg, best_acc1)
...
def train(train_loader, model, criterion, optimizer, epoch, args, tf_writer):
    ...
    progress = ProgressMeter(len(train_loader), batch_time, data_time, losses, top1,
                             top5, average, prefix="Epoch: [{}]".format(epoch))
...
def validate(val_loader, model, criterion, args, tf_writer=None, epoch=0):
    ...
    if tf_writer is not None:
        ...
        tf_writer.add_scalar('acc/test_top5', average.avg, epoch)

    return top1.avg, average.avg


def save_checkpoint(state, is_best, args):
    filename = '{}/{}/glide-ckpt.pth.tar'.format(args.root_model, args.store_name)
